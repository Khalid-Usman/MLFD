...
class FrankWolfeCoreset(object):
    def __init__(self, P, w, epsilon, coresetSize):
        self.P = P
        self.n = P.shape[0]
        self.w = w
        self.Q = P.T
    ...
    def term(self, x, P, x_k, j):
        return np.linalg.norm(
            np.sum(np.einsum('ij,j->ij', self.Q, (self.w - x_k - x * (np.roll(self.e_1, j) - x_k)).flatten()),
                   axis=1)) ** 2
    ...
    def applyFrankWolfe(self):
        ...
        for i in range(self.n):
            ...
            non_zero = np.where(x_k > 0)[0].shape[0]
            if non_zero == self.T:
                return self.P, x_k
